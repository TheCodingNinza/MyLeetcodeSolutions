...
public class BestTimeToBuyAndSellStockIV {
    ...
    private static Map<Pair<Integer,Integer>,Integer> dp;
    private static int count;
    public static int maxProfit(int k, int[] prices) {
        dp = new HashMap<>();
        int ans = recursion(k,prices,prices.length-1);
        return ans;
    }

    private static int recursion(int k,int[] prices,int start){
        count++;
        if(k <= 0){
            return 0;
        }
        ...
        if(dp.containsKey(new Pair<>(start,k))){
            return dp.get(new Pair<>(start,k));
        }
        int[] temp = new int[start+1];
        for (int j = start-1; j >= 0 ; j--) {
            if(prices[start] >= prices[j]){
                int tmp;
                if(dp.containsKey(new Pair<>(j-1,k-1))){
                    tmp = dp.get(new Pair<>(j-1,k-1));
                }else{
                    tmp = recursion(k-1,prices,j-1);
                }
                temp[j] = (prices[start] - prices[j])+tmp;
            }
        }
        temp[start] = recursion(k,prices,start-1);
        int max = myMax(temp);
        dp.put(new Pair<>(start,k),max);
        return max;
    }

    private static int myMax(int[] subject){
